...
function handleLogin($conn, $data) {
    ...
    $sql = "SELECT id, name, email, cpf, phone, role, password_hash FROM usuarios WHERE email = '$email'";
...
function handleSignup($conn, $data) {
    ...
    // Validações
    if (empty($name) || empty($email) || empty($cpf) || empty($phone) || empty($password)) {
    ...
    $checkCpf = $conn->query("SELECT id FROM usuarios WHERE cpf = '$cpf'");
    ...
    $sql = "INSERT INTO usuarios (name, email, cpf, phone, password_hash, role) 
            VALUES ('$name', '$email', '$cpf', '$phone', '$passwordHash', 'user')";
